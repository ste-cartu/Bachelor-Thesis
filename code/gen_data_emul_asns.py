...
def Transfer2(kk, On, Ob, Oc, h, sigma8, ns) :
...
def Prim(kk, ln_As_1e10, ns) :

    As = np.exp(ln_As_1e10) / (10**10)
    k_piv = 0.05
...
# ipercubo: [Omega_nu,                  Omega_b,    Omega_c,   h,       ln_10As,    ns]
inf =       [0.06/(93.14*(0.8**2)),     0.04,       0.23,      0.6,     2.5,        0.8]
sup =       [1/(93.14*(0.6**2)),        0.06,       0.29,      0.8,     3.5,        1.1]
input = qmc.scale(sample, inf, sup).T
np.save('../files/train_in_asns_emul_' + str(nc), input)
...
# ipercubo con nc combinazioni
sampler_v = qmc.LatinHypercube(d=6)
...
for c in range(nc) :
    ...
    prim[c,:] = Prim(kk, input[4,c], input[5,c])
    tf2[c,:] = Transfer2(kkh, input[0,c], input[1,c], input[2,c], h, sigma8, input[5,c])
...
np.save('../files/train_out_asns_emul_' + str(nc), output)
np.save('../files/train_pkz_asns_emul_' + str(nc), pkz)
np.save('../files/train_mu2_asns_emul_' + str(nc), mu2)
np.save('../files/train_prim_asns_emul_' + str(nc), prim)
np.save('../files/train_tf2_asns_emul_' + str(nc), tf2)
...
for c in range(ncv) :
    ...
    prim_v[c,:] = Prim(kk, input_v[4,c], input_v[5,c])
    tf2_v[c,:] = Transfer2(kkh, input_v[0,c], input_v[1,c], input_v[2,c], h, sigma8, input_v[5,c])
...
np.save('../files/val_out_asns_emul_' + str(ncv), output_v)
np.save('../files/val_pkz_asns_emul_' + str(ncv), pkz_v)
np.save('../files/val_mu2_asns_emul_' + str(ncv), mu2_v)
np.save('../files/val_prim_asns_emul_' + str(ncv), prim_v)
np.save('../files/val_tf2_asns_emul_' + str(ncv), tf2_v)
